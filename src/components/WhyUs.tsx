          ...
          {features.map((feature, idx) => {
            ...
            return (
              <div key={feature.titleKey} className={`card-hover text-center p-8 sm:p-10 rounded-xl sm:rounded-2xl bg-card-bg ${visible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: `${idx * 150}ms` }}>
                <div className="w-14 h-14 sm:w-20 sm:h-20 mx-auto mb-5 sm:mb-8 rounded-full bg-gradient-to-br from-brand/20 to-brand-light/10 flex items-center justify-center">
                  <Icon size={26} className="text-brand sm:w-8 sm:h-8" />
                </div>
