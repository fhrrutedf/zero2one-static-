    ...
    <section id="services" ref={sectionRef} className="py-14 sm:py-20 lg:py-28 section-dark-alt section-gold-accent-top">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className={`text-center max-w-2xl mx-auto mb-10 sm:mb-16 ${visible ? 'animate-fade-in-up' : 'opacity-0'}`}>
          ...
          <p className="text-white/85 leading-relaxed text-sm sm:text-base font-semibold">{t('services_subtitle')}</p>
        </div>
          ...
          {services.map((service, idx) => {
            ...
            return (
              <div key={service.titleKey} className={`card-hover bg-card-bg rounded-xl sm:rounded-2xl p-5 sm:p-6 lg:p-8 ${visible ? 'animate-fade-in-up' : 'opacity-0'}`} style={{ animationDelay: `${idx * 100}ms` }}>
                ...
                <p className="text-white/85 leading-relaxed mb-4 sm:mb-6 text-xs sm:text-sm font-semibold">{t(service.descKey)}</p>
                <a
                  href="https://wa.me/966530307054"
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={() => metaEvents.contact(`service_${idx + 1}_whatsapp`)}
                  className="inline-flex items-center gap-1.5 sm:gap-2 px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-[#25D366]/10 text-[#25D366] font-semibold text-xs sm:text-sm hover:bg-[#25D366] hover:text-white transition-all duration-300 min-h-[44px]"
                >
